...
function TodaysProfits() {
  ...
  // Base API URL (for both localhost & production)
  ...
  const handleDateChange = async (date) => {
    ...
    try {
      const response = await fetch(
        `/profits/date?date=${formattedDate}`,
        {
          method: "GET",
          headers: {
  ...
  return (
    <div className="FirstDiv">
      ...
      {!loading && !error && selectedDateProfit ? (
          ...
          <center>
            <h4>
              ...
              <span style={{ fontSize: "1.25rem" }}>
                {new Date(
                  selectedDateProfit.date_of_profit
                ).toLocaleDateString()}
              </span>
            </h4>
          </center>
        ...
        !loading &&
        !error && <h4>No profit data available for the selected date</h4>
      )}
    </div>
  );
}
